...
type (
	...
	HttpMatch func(*HttpContext) bool
	HttpAcceptFunc func()
	...
	HttpConnect interface {
		Open() error
		Close() error
		Accept(call func(res http.ResponseWriter, req *http.Request)) error
...
//HTTP初始化
func (module *httpModule) init() {
	module.initRouter()
	module.initSession()
	module.initHttp()
}
//初始化路由驱动
func (module *httpModule) initRouter() {
	if Config.Http.Router == nil {
		//使用默认的由路连接
...
//初始化会话驱动
func (module *httpModule) initSession() {
...
//初始化http驱动
func (module *httpModule) initHttp() {

	module.httpConfig = Config.Http
	module.httpConnect = module.connect(module.httpConfig)

	//打开http连接
	err := module.httpConnect.Open()
	if err != nil {
		panic("打开http服务失败 " + err.Error())
	}
	...
	//开始HTTP
	module.httpConnect.Start(Config.Http.Port)
}



//HTTP退出
func (module *httpModule) exit() {
	module.exitRouter()
	module.exitSession()
	module.exitHttp()
}
//HTTP退出，路由器
func (module *httpModule) exitRouter() {
	//关闭路由
	if module.routerConnect != nil {
...
//HTTP退出，会话
func (module *httpModule) exitSession() {
	//关闭会话
	if module.sessionConnect != nil {
...
//HTTP退出，HTTP
func (module *httpModule) exitHttp() {
	//关闭HTTP
...
//连接驱动
func (module *httpModule) connect(config *httpConfig) (HttpConnect) {
	if httpDriver,ok := module.drivers[config.Driver]; ok {
		return httpDriver.Connect(config.Config)
	} else {
		panic("不支持的HTTP驱动： " + config.Driver)
	}
}
...
//注册驱动
func (module *httpModule) Driver(name string, driver HttpDriver) {
	module.driversMutex.Lock()
	defer module.driversMutex.Unlock()

	if driver == nil {
		panic("http: Register driver is nil")
	}
	if _, ok := module.drivers[name]; ok {
		panic("http: Registered driver " + name)
	}
